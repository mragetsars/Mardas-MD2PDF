...
آیا خروجی PDF برای `version 1.13.34` و شماره ۱۴۰۵ پایدار است؟ پاسخ: بله؛ جدول زیر باید RTL، mixed-script، و mixed-number hooks را فعال کند.

| بخش نمونه | مقدار | انتظار در PDF |
| :--- | :--- | :--- |
| شماره فارسی | ۱۴۰۵ | عدد فارسی با متن RTL پایدار بماند. |
| نسخه فنی | version 1.13.34 و ۱.۹.۹ | Latin/Persian numerals در یک سلول خوانا بمانند. |
...
7. Use the default PDF-like preview for renderer-backed HTML with page size, margins, and auto-fit scaling; switch to Fast preview when you need instant browser-local feedback while editing.
...
> [!IMPORTANT]
> Studio now defaults to an auto-scaling PDF-like renderer preview with page size and margins. Fast preview remains useful for instant editing, and the final page breaks are still determined by the backend renderer and Chromium print layout during PDF export.
