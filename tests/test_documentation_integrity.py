...
def test_guides_start_with_valid_front_matter():
    for guide in GUIDES:
        ...
        assert metadata.get("version") == "1.13.3"
...
def test_guides_share_mardas_appearance_contract():
    ...
    for guide in GUIDES:
        ...
        assert metadata.get("branding", {}).get("mode") == "full"
        assert "brand" not in metadata

    assert (ROOT / "src/mardas_md2pdf/assets/Mardas.png").exists()
...
def test_guides_include_persian_rtl_live_smoke_samples():
    ...
    assert "version 1.13.3" in en
    assert "version 1.13.3" in fa
    assert "۱۴۰۵" in en
    assert "۱۴۰۵" in fa
